...
deliveryOrderSchema.virtual('totalItems').get(function() {
  return this.lines.reduce((total, line) => total + line.quantity, 0);
});

// Virtual for total value
deliveryOrderSchema.virtual('totalValue').get(function() {
...
deliveryOrderSchema.pre('save', async function(next) {
  if (this.isNew && !this.referenceNo) {
    ...
    this.referenceNo = `DEL-${year}-${String(count + 1).padStart(4, '0')}`;
  }
  next();
});
